...
function buildPrompt(texts, source, target) {
  ...
  return [
    `You are a professional translator. Translate ${texts.length} text blocks from ${source} to ${target}.`,
    ``,
    ...
    `- Translate each block as ONE complete unit — reorder words freely to make the target-language sentence natural.`,
    `- Do NOT translate word-by-word. Produce fluent, idiomatic ${target}, as a human translator would.`,
    `- If translating to Japanese: use natural Japanese word order, particles, and register (です・ます by default).`,
    ``,
    `PLACEHOLDER RULES (VERY IMPORTANT):`,
    `- Some blocks contain placeholders like ⟦0⟧, ⟦1⟧, ⟦2⟧. These represent inline HTML elements`,
    `  (e.g. <code>, <a>, <strong>) that must survive translation intact.`,
    `- Keep EVERY placeholder in the output, exactly as ⟦N⟧, with the same N.`,
    `- You MAY move placeholders around to fit natural ${target} word order — that is the whole point.`,
    `- Do NOT translate the numbers, do NOT add spaces inside ⟦N⟧, do NOT invent new placeholder numbers.`,
    ``,
    `FORMATTING RULES:`,
    `- If a block is already in ${target}, output it unchanged (still wrapped in the delimiter format).`,
    `- Preserve punctuation and important whitespace.`,
    `- Do NOT add commentary, notes, headers, or explanations outside the delimiter blocks.`,
    `- Do NOT wrap the output in markdown code fences.`,
    ``,
    `INPUT (${texts.length} blocks):`,
    numbered,
    ``,
    `Now output ${texts.length} translated blocks in the delimiter format:`,
  ].join("\n");
}
